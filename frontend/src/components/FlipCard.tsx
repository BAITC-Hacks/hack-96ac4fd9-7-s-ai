...
import {
  animate, motion, useMotionTemplate, useMotionValue, useReducedMotion, useSpring, useTransform,
} from 'motion/react';
import type { AnimationPlaybackControls, MotionStyle } from 'motion/react';
import './FlipCard.css';
...
interface Grip { id: number; x: number; y: number; base: number; moved: boolean; slop: number; hist: { t: number; v: number }[] }
...
export interface FlipCardProps {
  ...
  glare?: boolean;
  glareOpacity?: number;
  /** Glare colour as "r, g, b"; white glare is invisible on light cards. */
  glareRgb?: string;
  hoverScale?: number;
  perspective?: number;
  stiffness?: number;
  damping?: number;
  /** Pixels; omitted means the card fills its container. */
  width?: number;
  height?: number;
  radius?: number;
  background?: string;
...
export default function FlipCard({
  front = null, back = null, flipped, defaultFlipped = false, onFlipChange,
  axis = 'y', flipOnClick = true, draggable = true, dragDistance = 0,
  tilt = true, tiltMax = 12, glare = true, glareOpacity = 0.22, glareRgb = '255, 255, 255', hoverScale = 1.03,
  perspective = 1100, stiffness = 170, damping = 20,
  width, height = 400, radius = 22, background = '#27272a', color = '#f5f5f5',
  shadow = true, shadowColor = '#000000', shadowOpacity = 0.45,
  disabled = false, ariaLabel = 'Flip card', className = '',
}: FlipCardProps) {
  ...
  const onPointerDown = (e: PointerEvent<HTMLDivElement>) => {
    ...
    spin.current?.stop();
    grip.current = {
      id: e.pointerId, x: e.clientX, y: e.clientY, base: turn.get(), moved: false,
      slop: e.pointerType === 'touch' ? SLOP.coarse : SLOP.fine, hist: [],
    };
  ...
  // Keyboard-synthesised clicks have detail 0; pointer clicks are handled on pointerup.
  const onClick = (e: MouseEvent<HTMLDivElement>) => {
    if (!disabled && e.detail === 0) flip(true);
  };

  // Glare coordinates are unitless (0–100); the stylesheet converts them to percentages.
  const rotorStyle = {
    transform: axis === 'x' ? turnX : turnY, '--fc-gx': gx, '--fc-gy': gy, '--fc-sheen': sheen,
  } as MotionStyle;
  const rootStyle = {
    ...(width === undefined ? {} : { '--fc-w': `${width}px` }),
    '--fc-h': `${height}px`, '--fc-radius': `${radius}px`, '--fc-bg': background, '--fc-ink': color,
    '--fc-shadow': shadowColor, '--fc-shadow-o': shadowOpacity, '--fc-glare': glareOpacity, '--fc-glare-rgb': glareRgb,
  } as CSSProperties;
  ...
  return <div
    ...
    style={rootStyle}
  >
    {shadow ? <motion.span className="flip-card__shadow" aria-hidden="true"
      style={axis === 'x' ? { scaleY: spread, opacity: shade } : { scaleX: spread, opacity: shade }} /> : null}
