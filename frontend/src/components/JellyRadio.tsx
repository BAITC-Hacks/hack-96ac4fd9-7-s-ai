...
export interface JellyRadioItem { value: string; label: ReactNode; icon?: ReactNode; disabled?: boolean }
type Size = 'sm' | 'md' | 'lg';
interface ChipMotion { x: MotionValue<number>; sx: MotionValue<number>; sy: MotionValue<number> }

const DEFAULT_ITEMS = ['Off', 'Low', 'Medium', 'High', 'Max'];
const SIZES: Record<Size, [number, number, number]> = { sm: [28, 12, 12], md: [36, 13, 16], lg: [44, 14, 20] };

const spring = (k: number, m: number, bounce: number) => ({
  type: 'spring' as const, stiffness: k, damping: 2 * Math.sqrt(k * m) * (1 - bounce), mass: m,
});

const Chip = forwardRef<HTMLButtonElement, { mv: ChipMotion } & HTMLMotionProps<'button'>>(function Chip({ mv, children, ...rest }, ref) {
  const transform = useTransform(() => `translateX(${mv.x.get()}px) scale(${mv.sx.get()}, ${mv.sy.get()})`);
  return <motion.button ref={ref} style={{ transform }} {...rest}>{children}</motion.button>;
});
...
export default function JellyRadio({
  items = DEFAULT_ITEMS, value, defaultValue, onChange,
  chipColor = '#27272a', activeColor = '#f5f5f5', textColor = '#f5f5f5', activeTextColor = '#18181b',
  size = 'md', gap = 8, radius = 18, swell = 0.2, barge = 6, shrink = 0.05, jelly = 1, bounce = 0.25,
  stagger = 22, stiffness = 580, disabled = false, ariaLabel = 'Options', className = '',
}: JellyRadioProps) {
  ...
  const mvFor = (i: number): ChipMotion => {
  ...
  // Neighbours are pushed aside by the swollen active chip; farther chips react later and softer.
  ...
  // Padding leaves room for the swell so chips never overflow the group.
  const measure = () => {
    const group = groupRef.current;
    if (!group) return;
  ...
  useEffect(() => () => mvs.current.forEach((mv) => {
    mv.x.destroy();
    mv.sx.destroy();
    mv.sy.destroy();
  }), []);
  ...
  return <div
    ...
    style={{
      '--jr-chip': chipColor, '--jr-active': activeColor, '--jr-text': textColor, '--jr-active-text': activeTextColor,
      '--jr-gap': `${gap}px`, '--jr-radius': `${radius}px`, '--jr-h': `${h}px`, '--jr-font': `${font}px`, '--jr-px': `${px}px`,
    } as CSSProperties}
  >
    {list.map((it, i) => <Chip
      key={it.value}
