...
(() => {
  ...
  const labels = {
    kk: {
      home: "Басты бет", planner: "Менің іс-шарам", skip: "Мазмұнға өту", match: "Іріктеу", catalog: "Каталог", saved: "Таңдаулылар", compare: "Салыстыру", history: "Тарих", profiles: "профиль",
    ...
    ru: {
      home: "Главная", planner: "Моё событие", skip: "Перейти к содержимому", match: "Подбор", catalog: "Каталог", saved: "Избранное", compare: "Сравнение", history: "История", profiles: "профилей",
  ...
  function sendToMatch(query) {
    ...
    try { sessionStorage.setItem("firebird.prefill", JSON.stringify(prefill)); location.href = "/match"; }
    catch { toast(t("sessionFallback")); }
  }

  async function catalogPage(version) {
    document.title = "Firebird — " + t("catalog");
    main.replaceChildren(titleBlock(t("catalogEyebrow"), t("catalogTitle"), t("catalogSubtitle"), link(t("openSelection") + " ↗", "/match", "portal-button secondary")));
  ...
  function historyPage() {
    ...
    main.replaceChildren(titleBlock(t("historyEyebrow"), t("historyTitle"), t("historySubtitle"), link(t("startMatch") + " ↗", "/match", "portal-button secondary")), el("p", "browser-note", t("browserNote")));
    const records = store.getHistory();
    if (!records.length) { empty(main, t("historyEmpty"), t("historyEmptyText"), t("startMatch"), "/match", "↺"); return; }
